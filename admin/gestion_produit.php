        ...
        if($_POST){
            ...
            if (empty($erreur)){
                if($_GET['action'] == 'update'){
                    $modifierProduit = $pdo->prepare("UPDATE produit SET id_produit = :id_produit, date_arrivee = :date_arrivee, date_derat = :date_derat, id_salle = :id_salle, prix = :prix WHERE id_produit = :id_produit ");
                    ...
                    $modifierProduit->bindValue(':date_derat', $_POST['date_derat'], PDO::PARAM_STR);
    ...
    <div class="row mt-5 offset-2">
        <div class="col-md-4">
            ...
            <select class="form-select" name="id_salle" id="id_salle">
                <?php $afficheSalles = $pdo->query("SELECT DISTINCT id_salle FROM salle ORDER BY id_salle ASC") ;?>
                ...
                <?php while( $salle = $afficheSalles->fetch(PDO::FETCH_ASSOC) ): ?>
                    ...
                    <?php foreach( $salle as $key => $value): ?>
                        <option class="text-dark" value="id_salle "<?= ($value == $id_salle ) ? 'selected' : "" ?>><?= $value ?></option>
                    <?php endforeach ; ?>
                <?php endwhile ; ?>
            </select>
        </div>
...
<?php echo '<pre>' ; print_r($salle) ; echo '</pre>'; ?>
